...
class PDFReport(FPDF):
    ...
    def add_table(self, df, title, col_widths=None):
        ...
        if df.empty:
            self.add_text("No hay datos para mostrar.")
            return
    ...
    def add_date_comparison_section(self, comparison_df):
        """
        Agrega una sección al informe que muestra la comparación de registros por fecha.
        """
        self.add_section_title("Comparación de Registros por Fecha (Últimos 5 Días)")
        if comparison_df is None or comparison_df.empty:
            self.add_text("No se encontraron datos para comparar los registros por fecha.")
            return

        # Formatear las fechas para mostrar
        comparison_df['extraction_date'] = comparison_df['extraction_date'].apply(lambda x: x.strftime('%Y-%m-%d'))

        # Reordenar columnas para presentación
        comparison_df = comparison_df[['extraction_date', 'count_snowflake', 'count_redshift', 'match']]

        # Añadir tabla con los datos de comparación
        self.add_table(
            comparison_df,
            title="Comparación de Registros por Fecha"
        )

        # Verificar si los conteos coinciden por fecha
        if comparison_df['match'].all():
            self.add_text("Los conteos de registros por fecha coinciden en ambas bases de datos.")
        else:
            self.add_text("Existen discrepancias en los conteos de registros por fecha entre Snowflake y Redshift.")
...
def generate_audit_report(
    ...
    snowflake_total,
    redshift_total,
    comparison_df,  # Nuevo parámetro para comparación de fechas
    ...
    Args:
        ...
        comparison_df (pd.DataFrame): DataFrame con la comparación de registros por fecha.
...
"""
    ...
    pdf.add_date_comparison_section(comparison_df)

    # Comparación de Columnas
    pdf.chapter_title("Comparación de Columnas entre Snowflake y Redshift")
    if columns_snowflake_df is not None and columns_redshift_df is not None:
...
"""
        ...
        pdf.add_text("No se pudieron comparar las columnas debido a errores en la obtención de la información.")

    # Análisis de Datos Más Frecuentes
    if frequent_data:
        pdf.add_frequent_data_section(frequent_data, top_n=3)
    else:
        pdf.add_text("No se pudo realizar el análisis de los datos más frecuentes.")
